...
# Single NAT gateway keeps cost down; fine for staging & a lite production.
...
resource "aws_security_group" "alb" {
  name        = "${var.name}-alb"
  description = "Public ALB - HTTP from anywhere (TLS terminates at CloudFront)."
  vpc_id      = aws_vpc.this.id
...
resource "aws_security_group" "api" {
  name        = "${var.name}-api"
  description = "API tasks - traffic only from the ALB."
  vpc_id      = aws_vpc.this.id
