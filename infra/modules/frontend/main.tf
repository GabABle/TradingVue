# Static SPA on S3, fronted by CloudFront. CloudFront also proxies /api/* to the
# ALB so the browser talks to a single HTTPS origin (no CORS, relative fetches).
...
resource "aws_cloudfront_distribution" "this" {
  ...
  comment             = "${var.name} frontend + API proxy"
  ...
  # /api/* : proxy to the ALB, never cache, forward everything.
